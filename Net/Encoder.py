...
class PCNN(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.cnn = nn.Conv1d(3 * opt.word_dim, opt.hidden_size, 3, padding=1)
        self.fc1 = nn.Linear(3 * opt.word_dim, 3 * opt.word_dim)
        self.fc2 = nn.Linear(4 * opt.pos_dim + opt.word_dim, 3 * opt.word_dim)
        self.hidden_size = opt.hidden_size
        self.lam = opt.lambda_pcnn
        self.init_weight()
...
class SAN(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.fc1 = nn.Linear(3 * opt.word_dim, 3 * opt.word_dim)
        self.fc2 = nn.Linear(4 * opt.pos_dim + opt.word_dim, 3 * opt.word_dim)
        self.fc1_att = nn.Linear(3 * opt.word_dim, 3 * opt.word_dim)
        self.fc2_att = nn.Linear(3 * opt.word_dim, 3 * opt.word_dim)
        self.lam = opt.lambda_san
        self.init_weight()
